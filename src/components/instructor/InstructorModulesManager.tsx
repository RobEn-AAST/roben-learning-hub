...
export function InstructorModulesManager() {
  ...
  const loadCurrentUser = async () => {
    try {
      ...
      if (user) {
        setCurrentUser(user);
        // Pre-fill instructor_id in lesson form
        setLessonFormData(prev => ({ ...prev, instructor_id: user.id }));
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (editingModule) {
          ...
          // Log module creation
          const createdModule = result; // FIXED: API returns module directly, not wrapped
          ...
          // FIXED: Update all states synchronously before re-rendering
          ...
          // FIXED: Load modules in background (don't await to prevent blocking)
          loadModules();
          loadInitialData();
        }
      } else {
  ...
  const handleLessonSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        const createdLesson = result; // FIXED: API returns lesson directly, not wrapped
