...
import { StudentProgressData } from "@/types/progressTracking";
...
export const courseInstructorService = {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(
            `API returned HTML instead of JSON (Status: ${response.status}). This usually means the endpoint is not found or there's an authentication issue.`
          );
        }

        throw new Error(
          errorData.error || "Failed to fetch course instructors"
        );
      }
      ...
      try {
        ...
        throw new Error(
          "API returned HTML instead of JSON. This usually indicates a server error or authentication issue."
        );
      }
      ...
      throw new Error(
        error instanceof Error
          ? error.message
          : "Failed to get course instructors"
      );
    }
  },
  ...
  async getAvailableInstructors(): Promise<InstructorProfile[]> {
    try {
      const response = await fetch(
        "/api/admin/course-instructors?type=available"
      );

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Failed to fetch available instructors"
        );
      }
      ...
      throw new Error(
        error instanceof Error
          ? error.message
          : "Failed to get available instructors"
      );
    }
  },
  ...
  async getUnassignedInstructors(
    ...
    return allInstructors.filter(
      (instructor) => !assignedIds.includes(instructor.id)
    );
  },
  ...
  // Get all students and their progress for a course (instructor access only)
  async getCourseStudentProgress(
    course_id: string
  ): Promise<StudentProgressData> {
    try {
      const response = await fetch(
        `/api/instructor/courses/${course_id}/students`,
        {
          method: "GET",
          credentials: "same-origin",
        }
